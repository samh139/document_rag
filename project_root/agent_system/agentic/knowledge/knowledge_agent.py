...
import requests
from agent_system.agentic.memory_team.stm.store import set_clarification_context
...
class KnowledgeAgent(RoutedAgent):
    ...
    async def handle_refined_query(
        ...
        logger.info(
            f"[KnowledgeAgent] Routing query for session={message.session_id}"
        )
        ...
        logger.info(
            f"[KnowledgeAgent] Published routing "
    ...
    clusters: List[dict],
        ...
        if not confident_ann:
            logger.info(
                "[KnowledgeAgent] ANN not confident → writing clarification context"
            )
        ...
        if not summary_allows:
            logger.info(
                "[KnowledgeAgent] Summary gate blocked routing → writing clarification context"
            )
            return self._ambiguous(
                message,
                confidence=top["score"],
